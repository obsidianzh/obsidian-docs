我们相信 [[zh/Obsidian/Obsidian#跟其他软件有何不同|你拥有并且掌控你的数据]] 。你的笔记将按照 markdown 格式进行存储，后者是一个基于纯文本的开放文件格式。所以，任何能够打开文本文件的电脑，都能够打开这份文件。你也可以用其他的软件来编辑这些文件，即使 Obsidian 正在使用它。
